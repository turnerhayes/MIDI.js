...
const Synesthesia = {
  ...
  map(type) {
    ...
    const syn = Synesthesia.data;
    const colors = syn[type] || syn["D. D. Jameson (1844)"];
    for (let note = 0, pclr, H, S, L; note <= 88; note ++) { // creates mapping for 88 notes
      if (colors.data) {
        data[note] = {
        ...
        data[note] = {
          hex: ColorSpace({H, S, L}, "HSL>RGB>HEX>W3"),
          hsl: "hsla(" + H + "," + S + "%," + L + "%, 1)",
        };
