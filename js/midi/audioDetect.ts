...
const canPlayThrough = (src) => { // check whether format plays through
	pending ++;
...
const audioDetect = (onsuccess) => {
	...
	const vorbisSupport = audio.canPlayType('audio/ogg; codecs="vorbis"');
	const vorbis = (vorbisSupport === "probably" || vorbisSupport === "maybe");
	const mpegSupport = audio.canPlayType("audio/mpeg");
	const mpeg = (mpegSupport === "probably" || mpegSupport === "maybe");
